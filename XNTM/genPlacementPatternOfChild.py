...
def main():
                    ...
                    if right_end < GridSize and lower_end < GridSize :
                        ...
                        for y in range(ref_y,lower_end+1):
                            for x in range(ref_x,right_end+1):
                                ...
                                grid[y][x]= 1
                        if cnt >0 :
                            if keystr(cnt) not in res[pM][cM]:
                                res[pM][cM][keystr(cnt)] = []
                            ...
                            res[pM][cM][keystr(cnt)].append(inserted_data)
                            cnt_pattern += 1
    ...
    for i in range(len(Mixer)):
        ...
        for j in range(len(Mixer)):
            tmp = 0
            for cnt in range(7):
                if keystr(cnt) in res[Mixer[i]][Mixer[j]] :
                    c += len(res[Mixer[i]][Mixer[j]][keystr(cnt)])
                    tmp += len(res[Mixer[i]][Mixer[j]][keystr(cnt)])
    ...
    for p in range(len(Mixer)):
        for c in range(len(Mixer)):
            ...
            for cnt in range(7):
                if keystr(cnt) in res[Mixer[p]][Mixer[c]]:
                    NoFlushingPatterns = copy.deepcopy(res[Mixer[p]][Mixer[c]][keystr(cnt)])
                        ...
                        if item["flushing_cell"] == [] :
                            ...
                            for i in range(2**len(alt)):
                                ...
                                if skip :
                                    ...
                                    snumLeftDrops = keystr(len(DropLeftCells))
            ...
            cnt_flushpattern = 0
            for cnt in range(7):
                if keystr(cnt) in res_buffer[Mixer[p]][Mixer[c]]:
                    for insert in res_buffer[Mixer[p]][Mixer[c]][keystr(cnt)]:
                        if keystr(cnt) not in res[Mixer[p]][Mixer[c]] :
                            res[Mixer[p]][Mixer[c]][keystr(cnt)] = []
                        ### print(insert)
                        res[Mixer[p]][Mixer[c]][keystr(cnt)].append(insert)
                        cnt_pattern += 1
                        cnt_flushpattern += 1
    ...
    for pm in range(len(Mixer)):
        ...
        while q:
            ...
            if rSize < numCells(PM):
                if keystr(rSize) not in res_buffer_r[PM]:
                    res_buffer_r[PM][keystr(rSize)] = []
                insert_data = sorted(r)
                if insert_data not in res_buffer_r[PM][keystr(rSize)]:
                    res_buffer_r[PM][keystr(rSize)].append(insert_data)
        ...
        for n in range(6):
            if keystr(n) in res_buffer_r[PM]:
                module_name = "r"+keystr(n)
                res[PM][module_name] = {}
                res[PM][module_name][keystr(n)] = []
                for pattern in res_buffer_r[PM][keystr(n)]:
                    sorted_pattern = sorted(pattern)
                    data = {"overlapping_cell":sorted_pattern,"flushing_cell":[]}
                    res[PM][module_name][keystr(n)].append(data)
                    grid = gengrid(PM)
    ...
    with open("XNTM/data/PlacementChild.json","w") as f:
        keystr_res = json.dump(res, f,separators=(',', ':')) 
