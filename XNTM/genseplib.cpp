...
int main() {
  ...
  json lib;
  for (string PM : Mixer) {
    unordered_map<string,unordered_set<string>> ResKey; 
    lib[PM] = json::object();
    ...
    for (auto ratio : prov_ratio[PM]) {
      ...
      ResKey.insert({s_ratio,unordered_set<string>()});
      ...
      for(auto key:ResKey[s_ratio]){
          ofstream writing_file;
          string WriteFileName = "XNTM/sepdata/lib"+PM+s_ratio+key+".json"; 
          now = chrono::system_clock::now(); 
          diff = chrono::duration_cast<chrono::hours>(now-start).count();
          cout<<WriteFileName<<"の書き出し.ここまでの経過時間:"<<diff<<"分"<<endl; 
          writing_file.open(WriteFileName,ios::out); 
          writing_file << lib[PM][s_ratio][key] <<endl; 
          writing_file.close(); 
      }
    }
  }
