...
def main():
    ...
    ReadFilePath = Path("XNTM/data","placement.json")
    readfile = ReadFilePath.open()
                ...
                #for combo in RatioOrderedCombo:
                    ...
                    if buffer[len(ratio)]:
                        for data in buffer[len(ratio)]:
                            ...
                            if str(modulecnt) not in lib:
                                lib[str(modulecnt)] = []
                            if data not in lib[str(modulecnt)]:
                                lib[str(modulecnt)].append(data)
                    else :
                        print("ホンマ，ありえへんてぇ~!! in genlib.py",file=sys.stderr)
                    
            ### output lib to json file
            FileName = "lib"+PM+str(ratio)+".json"
            now = datetime.datetime.now()
            diff = now-start
            print("終了:PM:{},ratio:{} ,経過時間: {}".format(PM,ratio,diff))
            WriteFilePath = Path("XNTM/data",FileName)
            with WriteFilePath.open(mode='w') as f:
